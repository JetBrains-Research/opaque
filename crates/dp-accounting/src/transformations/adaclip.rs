...
//!
//! Provides the `combined_sensitivity` formula for adaptive clipping with
...
mod tests {
    ...
    #[test]
    fn test_combined_sensitivity_basic() {
        // z=1, σ_b=∞ → z̃ ≈ 1/z = 1.0
        let result = combined_sensitivity(1.0, 1e10);
        assert!((result - 1.0).abs() < 1e-5);
    }

    #[test]
    fn test_combined_sensitivity_equal_contribution() {
        // z=1, σ_b=0.5 → z̃ = sqrt(1 + 1) = sqrt(2)
        let result = combined_sensitivity(1.0, 0.5);
        assert!((result - std::f64::consts::SQRT_2).abs() < 1e-10);
    }

    #[test]
    fn test_combined_sensitivity_large_quantile_noise() {
        // Large σ_b → dominated by 1/z²
        let z = 0.5;
        let result = combined_sensitivity(z, 1e6);
        let expected = 1.0 / z;
        assert!((result - expected).abs() < 1e-6);
    }

    #[test]
    fn test_combined_sensitivity_symmetry() {
        // combined_sensitivity is symmetric except for the 4x factor on σ_b
        let a = combined_sensitivity(1.0, 2.0);
        let b = combined_sensitivity(2.0, 1.0);
        // Not symmetric — verify different
        assert!((a - b).abs() > 0.01);
    }

    #[test]
    fn test_combined_sensitivity_always_positive() {
        for &z in &[0.1, 0.5, 1.0, 2.0] {
            for &sb in &[0.1, 0.5, 1.0, 10.0] {
                let result = combined_sensitivity(z, sb);
