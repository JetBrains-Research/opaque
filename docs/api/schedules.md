...
- pure curves: `constant_schedule`, `cosine_schedule`,
  `inverse_sqrt_schedule`, `one_minus_sqrt_schedule`;
- composition primitives: `with_warmup`, `with_restarts`;
- re-exports from
  [`torchopt.schedule`](https://torchopt.readthedocs.io/en/latest/api/api.html#schedules):
  `linear_schedule`, `polynomial_schedule`, `exponential_decay`.

Every public function returns a plain `Callable[[int], float]` that
plugs straight into `torchopt.adamw(lr=...)`,
`torchopt.adam(lr=...)`, `torchopt.sgd(lr=...)`, or
[`opaque.dpsgd.adamw_bc`](optimizers.md).  TorchOpt's
`scale_by_neg_lr` advances the schedule via the optimizer-state step
counter — no manual `.step()` call.
...
```python
from opaque.dpsgd.schedules import constant_schedule, with_warmup
...
```python
from opaque.dpsgd.schedules import cosine_schedule
...
```python
from opaque.dpsgd.schedules import inverse_sqrt_schedule
...
```python
from opaque.dpsgd.schedules import one_minus_sqrt_schedule
...
Schedules in this module and in `torchopt.schedule` return
`init_value` for `step < transition_begin`, so configuring a decay
with `transition_begin == transition_steps` of the warmup produces
the standard "warmup, then decay" shape:
...
sched = with_warmup(
    torchopt.schedule.linear_schedule(base_lr, 0.0, decay_steps, transition_begin=W),
    transition_steps=W,
)
...
- [`torchopt.schedule`](https://torchopt.readthedocs.io/) — `linear_schedule`, `polynomial_schedule`, `exponential_decay`.
