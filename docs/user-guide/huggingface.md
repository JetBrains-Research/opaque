...
For `DPTrainer`-specific distributed arguments, eval behavior, and Hub rank
gating, see [Distributed DPTrainer](distributed-trainer.md).

## Hyperparameter search

`DPTrainer` does **not** implement `hyperparameter_search`. Running a
sweep over private data is itself an adaptive use of the dataset's
privacy budget, and the silent composition that `Trainer.hyperparameter_search`
performs would obscure that. Drive sweeps from your orchestration layer
(Ray, Optuna, W&B) directly: spawn one fresh `DPTrainer` per trial,
read each trial's ε / δ from its `accountant.json`, and account for
sweep-level composition explicitly.

## Hub publishing

`DPTrainer` does **not** push to the HF Hub. Save the model locally
(`trainer.save_model(out_dir)` — which writes `accountant.json` alongside
the weights) and upload from your orchestration layer via
`huggingface_hub.upload_folder`. Keeping the upload out of the trainer
ensures the privacy accountant always travels with whatever artefact
the user publishes.
