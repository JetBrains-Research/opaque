...
| `torch_compile` | `False` | Compiles the tensor-only per-microbatch `vmap(grad)+clip+reduce` kernel with `fullgraph=True` and a dynamic leading dimension, avoiding specialization for each realized Poisson batch size. |
