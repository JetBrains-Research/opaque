...
| `router_load_max_tokens` | `None` | Public bound on the valid tokens of one row; the collator (tokenizer truncation) must enforce it, and the trainer raises on a longer row as a configuration guard. `None` resolves to the SFT / DPO `max_length` or, with a warning, to the model's `max_position_embeddings`: that fallback keeps the bound valid but, with rows much shorter than the context length, scales the release signal and the surrogate weight down by `(T_bar_true / max_position_embeddings)^2`, so set it to the collator's row length. |
