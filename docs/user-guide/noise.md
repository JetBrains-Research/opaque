...
MF noise runs eagerly, and the first gradient template selects the provider.
Strategy recipes and their `coefficients(...)` queries stay host-side and return
NumPy arrays; runtime samples, correlation buffers, outputs, `PerGroup`
allocations, and paired second-moment streams stay native to that provider.
`compute_dtype=None` computes at its `float32` and casts each output leaf back
to its input dtype and placement.

Thread the returned `MFNoiseState` or `SecondMomentMFNoiseState` into the next
call, and checkpoint it with `opaque.serialization.state_dict`; restoring
against a matching state template continues the saved key, step counter, and
correlation buffers.
