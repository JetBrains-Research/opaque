...
for epoch_indices in sampler:
    batch = dataset[epoch_indices]
    grads, clip_state = grad_fn(params, batch, state=clip_state)
    noisy_grads, noise_state = noise_fn(grads, noise_state)
    params = update(params, noisy_grads)

    acct = acct | step
    if acct.epsilon_at(1e-5) > 3.0:
        break

# 5. Report final privacy guarantee
print(f"Privacy: (\u03b5={acct.epsilon_at(1e-5):.2f}, \u03b4=1e-5)")
```
