...
`n_steps`, `min_sep`, and `max_participations` are supplied to
`mf_gaussian_noise` and the matching accounting amplifier. The strategy uses
that context when its coefficients, sensitivity, or Gram matrix are queried.
...
The noise function derives each fresh IID draw from the explicit key and
computes the linear combination defined by the BISR coefficients.
