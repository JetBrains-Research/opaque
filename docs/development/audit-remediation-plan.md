...
| ✅ Require flat dict for per-group clip/noise; raise on group/key mismatch | `engine/clipping/{_pytree,_per_group}.py` (+ dpsgd/dpftrl PerGroup noise) | Nested containers previously skipped leaves (unclipped / wrong σ); flat `named_parameters` is the contract |
