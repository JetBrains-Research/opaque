...
- **Balls-in-Bins schedule:** `BallsInBinsSampler` now emits empty bin slots instead of compacting them away, so the trainer executes all `num_bins` slots the accountant charges per epoch (OPQ-186).
