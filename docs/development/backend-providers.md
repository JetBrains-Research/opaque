...
Install `opaque-engine` together with the provider wheel an application
uses, or install `opaque` for the default Torch bundle. Torch is the only
shipped provider today; the dispatch layer, `KnownBackend` names, and the
install guidance below already reserve the JAX and MLX slots.
...
dataclasses, arrays, and model type hierarchies. The first recognized
Torch, JAX, or MLX value loads the matching provider wheel (`opaque-torch`
today; the JAX and MLX loads raise a guided install error until those
wheels ship) and makes that backend sticky:
...
# Known providers can also be activated by name: set_backend("torch").

with use_backend(torch_backend()):
    result = selective_log_softmax(torch_logits, torch_indices)

# The previous sticky selection is restored here.
clear_backend()
```
...
The first-party capability matrix (Torch shipped; JAX/MLX columns record
the design targets for the planned providers) is:
