...
cfg = MellumConfig(vocab_size=128, hidden_size=64, intermediate_size=128, num_hidden_layers=2,
                   ...
                   output_router_logits=True, router_aux_loss_coef=0.001, attn_implementation="eager")
...
tr = Trainer(model=model, args=args, train_dataset=ds, data_collator=collate, callbacks=[cap])
...
json.dump(dict(res=res, auxs=auxs, aux_all=aux_all, n_items=int(n_items)), open("/tmp/claude-0/-home-user-opaque/a97cca5e-900f-5617-b2df-d73b55081cac/scratchpad/research/critic/expA.json","w"), indent=1)
