...
def apply_rotary_pos_emb(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor = None) -> torch.Tensor:
...
def vmap_phi3_attention_forward(
    ...
    **kwargs,
    ...
    if past_key_value is not None and not isinstance(past_key_value, VmapCompatibleDynamicCache):
        past_key_value = VmapCompatibleDynamicCache(past_key_value)
