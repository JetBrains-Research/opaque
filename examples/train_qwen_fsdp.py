...
def main():
    ...
    if is_main:
        ...
        mode = (
            "FSDP (Multi-GPU)"
            if use_fsdp
            else ("DDP-style (Multi-GPU)" if distributed else "Single GPU")
        )
        print(f"  Mode: {mode}")
    ...
    if use_fsdp:
        ...
        if is_main:
            ...
            print(
                "   Sharding strategy: FULL_SHARD (parameters + gradients + optimizer states)"
            )
            print("   Mixed precision: Enabled (reduces memory further)")
    ...
    # Create optimizer (TorchOpt functional optimizer)
    ...
    #    - After FSDP AllReduce + noise (same seed), all ranks have identical noisy_grads
    #    - Therefore, opt_state evolves identically on all ranks (no explicit sync needed)
    ...
    try:
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, tokens in enumerate(batches):
                ...
                if global_step == 0 and is_main:
                    print("Step 1: Compiling model (first step)...")
                    print("  This will take 30-60s, subsequent steps ~5-10s each\n")
                ...
                # ℹ️ opt_state is now identical on all ranks (pure function property)
        ...
        if is_main:
            print("\n" + "=" * 80)
            print("✅ DP-SGD LORA + FSDP TRAINING COMPLETE!")
            print("=" * 80)
            print("\nDistributed configuration:")
            print(f"  World size: {world_size} GPUs")
            print("  Sharding: FSDP FULL_SHARD")
            print(f"  Samples per rank: {samples_per_rank}")
