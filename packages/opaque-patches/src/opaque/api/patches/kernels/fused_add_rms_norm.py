...
class _FusedAddRMSNormBackward(torch.autograd.Function):
    ...
    def vmap(info, in_dims, dY, dS_out, S, W, RSTD, meta_i, offset_tensor):
        ...
        if dy_b != 0 or s_b != 0 or r_b != 0:
            ConfigurationError.raise_("dY, S, RSTD must be vmapped at dim 0")
        H = S.shape[-1]
        head_dy = dY.shape[:-1]
        B = dY.shape[0]
        dY_m = dY.reshape(-1, H)
        ...
        dComb, dW_summed = _fused_add_rms_norm_backward_triton(
            dY_m,
            dS_m,
            S_m,
            W,
    ...
    def vmap(info, in_dims, X, R, W, eps, offset, casting_mode, in_place):
        ...
        for i in range(2, 7):
            if in_dims[i] is not None:
                ConfigurationError.raise_("Only X and R may be batched under vmap")
        cm = _casting_mode_int(casting_mode)
        shape = X.shape
        H = shape[-1]
...
def opaque_fused_add_rms_norm(
    ...
    *,
    ...
    if not x.is_cuda:
        OperationError.raise_("opaque_fused_add_rms_norm Triton path requires CUDA")
