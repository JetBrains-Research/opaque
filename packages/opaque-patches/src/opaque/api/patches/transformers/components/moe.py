...
``fused`` (wired from the ``fused_moe`` patch gate, default ``kernels``) picks
the grouped-GEMM fast path — sparse Triton on CUDA bf16/fp16, ``torch._grouped_mm``
on MPS/CPU — while ``fused=False`` forces the dense ``Opaque_MoE``. All paths are
...
def _make_moe_experts_forward(original, *, fused=True):
    """Build a stacked-weight MoE experts ``forward`` using the Opaque kernel.

    ``fused`` (captured at patch time from the ``fused_moe`` gate) selects the
    ...
    def forward(self, hidden_states, top_k_index, top_k_weights):
        ...
        out = opaque_moe(
            x, self.gate_up_proj, self.down_proj, idx, weights, fused=fused
        )
