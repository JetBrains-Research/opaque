...
def apply_olmo3_patches(
    ...
    **kwargs
    ...
    """Apply Triton kernel patches for olmo3 model."""
    try:
        import transformers.models.olmo3.modeling_olmo3 as mod
    except ImportError:
        return
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'Olmo3MLP', None), _make_swiglu_mlp_forward, model)
    if fuse_rms_norm:
        _patch_forward(getattr(mod, 'Olmo3RMSNorm', None), _rmsnorm_fac_olmo2, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'Olmo3ForCausalLM', None), _make_fused_ce_causal_lm_forward, model)

    causal_lm_cls = getattr(mod, 'Olmo3ForCausalLM', None)
    if wrap_batchify:
