...
from opaque.patches.transformers.components.fused_add_rms_norm import _fused_add_rms_fac_gemma
...
def apply_gemma_patches(
    ...
    **kwargs
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'GemmaMLP', None), _make_geglu_exact_mlp_forward, model)
    if fuse_rms_norm:
        _patch_forward(getattr(mod, 'GemmaRMSNorm', None), _rmsnorm_fac_gemma, model)
    if fuse_add_rms_norm:
        _patch_forward(getattr(mod, 'GemmaDecoderLayer', None), _fused_add_rms_fac_gemma, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'GemmaForCausalLM', None), _make_fused_ce_causal_lm_forward, model)
