...
def apply_llama_patches(
    ...
    **kwargs
    ...
    if fuse_add_rms_norm:
        _patch_forward(getattr(mod, 'LlamaDecoderLayer', None), _fused_add_rms_fac_llama, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'LlamaForCausalLM', None), _make_fused_ce_causal_lm_forward, model)
