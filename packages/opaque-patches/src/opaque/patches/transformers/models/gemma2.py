...
from opaque.patches.transformers.components.attention import vmap_repeat_kv, vmap_eager_attention_forward_gemma2, apply_module_masking_patch


logger = logging.getLogger(__name__)

def apply_gemma2_patches(
    ...
    **kwargs
    ...
    """Apply Triton kernel patches for gemma2 model."""
    try:
        import transformers.models.gemma2.modeling_gemma2 as mod
    except ImportError:
        return
    ...
    if wrap_eager_attention:
        ...
        if hasattr(mod, 'eager_attention_forward'):
            mod.eager_attention_forward = vmap_eager_attention_forward_gemma2

    if fuse_swiglu:
        _patch_forward(getattr(mod, 'Gemma2MLP', None), _make_geglu_approx_mlp_forward, model)
    if fuse_rms_norm:
        _patch_forward(getattr(mod, 'Gemma2RMSNorm', None), _rmsnorm_fac_gemma2, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'Gemma2ForCausalLM', None), _make_fused_ce_causal_lm_forward, model)

    causal_lm_cls = getattr(mod, 'Gemma2ForCausalLM', None)
    if wrap_batchify:
