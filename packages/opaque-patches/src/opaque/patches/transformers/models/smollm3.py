...
def apply_smollm3_patches(
    ...
    **kwargs,
    ...
    if fuse_swiglu:
        _patch_forward(
            getattr(mod, "SmolLM3MLP", None), _make_swiglu_mlp_forward, model
        )
    if fuse_rms_norm:
        _patch_forward(getattr(mod, "SmolLM3RMSNorm", None), _rmsnorm_fac_llama, model)
    if fuse_add_rms_norm:
        _patch_forward(
            getattr(mod, "SmolLM3DecoderLayer", None), _fused_add_rms_fac_llama, model
        )
    if fuse_rope:
        if (
    ...
    if fuse_cross_entropy:
        _patch_forward(
            getattr(mod, "SmolLM3ForCausalLM", None),
            _make_fused_ce_causal_lm_forward,
            model,
        )
