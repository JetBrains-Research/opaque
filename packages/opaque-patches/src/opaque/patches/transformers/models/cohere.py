...
def apply_cohere_patches(
    ...
    **kwargs
    ...
    """Apply Triton kernel patches for cohere model."""
    try:
        import transformers.models.cohere.modeling_cohere as mod
    except ImportError:
        return
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'CohereMLP', None), _make_swiglu_mlp_forward, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'CohereForCausalLM', None), _make_fused_ce_causal_lm_forward, model)

    causal_lm_cls = getattr(mod, 'CohereForCausalLM', None)
    if wrap_batchify:
