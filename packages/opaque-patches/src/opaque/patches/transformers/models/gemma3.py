...
def apply_gemma3_patches(
    ...
    **kwargs
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'Gemma3MLP', None), _make_geglu_approx_mlp_forward, model)
    if fuse_rms_norm:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'Gemma3ForCausalLM', None), _make_fused_ce_causal_lm_forward, model)
