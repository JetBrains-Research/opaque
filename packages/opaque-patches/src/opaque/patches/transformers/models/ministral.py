...
def apply_ministral_patches(
    ...
    **kwargs
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'MinistralMLP', None), _make_swiglu_mlp_forward, model)
    if fuse_rms_norm:
        _patch_forward(getattr(mod, 'MinistralRMSNorm', None), _rmsnorm_fac_llama, model)
    if fuse_add_rms_norm:
        _patch_forward(getattr(mod, 'MinistralDecoderLayer', None), _fused_add_rms_fac_llama, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'MinistralForCausalLM', None), _make_fused_ce_causal_lm_forward, model)
