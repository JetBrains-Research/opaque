...
def apply_granite_patches(
    ...
    **kwargs,
    ...
    if swiglu:
        _patch_forward(
            getattr(mod, "GraniteMLP", None), _make_swiglu_mlp_forward, model
        )
    if rms_norm:
        _patch_forward(getattr(mod, "GraniteRMSNorm", None), _rmsnorm_fac_llama, model)
        _patch_forward(
            getattr(mod, "GraniteDecoderLayer", None), _fused_add_rms_fac_granite, model
        )
    if rope:
        if (
    ...
    if cross_entropy:
        _patch_forward(
            getattr(mod, "GraniteForCausalLM", None),
            _make_fused_ce_causal_lm_forward,
            model,
        )
