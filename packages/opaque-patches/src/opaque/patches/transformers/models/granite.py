...
from opaque.patches.transformers.components.fused_add_rms_norm import _fused_add_rms_fac_granite
...
def apply_granite_patches(
    ...
    **kwargs
    ...
    """Apply Triton kernel patches for granite model."""
    try:
        import transformers.models.granite.modeling_granite as mod
    except ImportError:
        return
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'GraniteMLP', None), _make_swiglu_mlp_forward, model)
    if fuse_rms_norm:
        _patch_forward(getattr(mod, 'GraniteRMSNorm', None), _rmsnorm_fac_llama, model)
    if fuse_add_rms_norm:
        _patch_forward(getattr(mod, 'GraniteDecoderLayer', None), _fused_add_rms_fac_granite, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'GraniteForCausalLM', None), _make_fused_ce_causal_lm_forward, model)

    causal_lm_cls = getattr(mod, 'GraniteForCausalLM', None)
    if wrap_batchify:
