...
def apply_exaone4_patches(
    ...
    **kwargs
    ...
    """Apply Triton kernel patches for exaone4 model."""
    try:
        import transformers.models.exaone4.modeling_exaone4 as mod
    except ImportError:
        return
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'Exaone4MLP', None), _make_swiglu_mlp_forward, model)
    if fuse_rms_norm:
        _patch_forward(getattr(mod, 'Exaone4RMSNorm', None), _rmsnorm_fac_olmo2, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'Exaone4ForCausalLM', None), _make_fused_ce_causal_lm_forward, model)

    causal_lm_cls = getattr(mod, 'Exaone4ForCausalLM', None)
    if wrap_batchify:
