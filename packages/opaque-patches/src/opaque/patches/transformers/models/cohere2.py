...
def apply_cohere2_patches(
    ...
    **kwargs
    ...
    """Apply Triton kernel patches for cohere2 model."""
    try:
        import transformers.models.cohere2.modeling_cohere2 as mod
    except ImportError:
        return
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'Cohere2MLP', None), _make_swiglu_mlp_forward, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'Cohere2ForCausalLM', None), _make_fused_ce_causal_lm_forward, model)

    causal_lm_cls = getattr(mod, 'Cohere2ForCausalLM', None)
    if wrap_batchify:
