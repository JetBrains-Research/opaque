...
def apply_qwen3_patches(
    ...
    **kwargs
    ...
    """Apply Triton kernel patches for qwen3 model."""
    try:
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'Qwen3MLP', None), _make_swiglu_mlp_forward, model)
    if fuse_rms_norm:
        _patch_forward(getattr(mod, 'Qwen3RMSNorm', None), _rmsnorm_fac_llama, model)
    if fuse_add_rms_norm:
        _patch_forward(getattr(mod, 'Qwen3DecoderLayer', None), _fused_add_rms_fac_llama, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'Qwen3ForCausalLM', None), _make_fused_ce_causal_lm_forward, model)

    causal_lm_cls = getattr(mod, 'Qwen3ForCausalLM', None)
    if wrap_batchify:
