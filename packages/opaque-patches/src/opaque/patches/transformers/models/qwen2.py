...
def apply_qwen2_patches(
    ...
    **kwargs
    ...
    if fuse_swiglu:
        _patch_forward(getattr(mod, 'Qwen2MLP', None), _make_swiglu_mlp_forward, model)
    if fuse_rms_norm:
        _patch_forward(getattr(mod, 'Qwen2RMSNorm', None), _rmsnorm_fac_llama, model)
    if fuse_add_rms_norm:
        _patch_forward(getattr(mod, 'Qwen2DecoderLayer', None), _fused_add_rms_fac_llama, model)
    if fuse_rope:
    ...
    if fuse_cross_entropy:
        _patch_forward(getattr(mod, 'Qwen2ForCausalLM', None), _make_fused_ce_causal_lm_forward, model)

    causal_lm_cls = getattr(mod, 'Qwen2ForCausalLM', None)
    if wrap_batchify:
