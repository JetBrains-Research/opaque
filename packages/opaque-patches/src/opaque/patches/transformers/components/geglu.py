...
def _make_geglu_exact_mlp_forward(original):
    ...
    def forward(self, x):
        ...
        from opaque.patches.kernels.geglu import Opaque_GeGLU_Exact
...
def _make_geglu_approx_mlp_forward(original):
    ...
    def forward(self, x):
        if not x.is_cuda:
            return original(self, x)
        from opaque.patches.kernels.geglu import Opaque_GeGLU_Approx
...
def _opaque_apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    from opaque.patches.kernels.rope_embedding import Opaque_RoPE_QK
...
def _opaque_causal_lm_loss(
    ...
    **kwargs,
    ...
    from opaque.patches.kernels.cross_entropy import Opaque_CrossEntropyLoss
