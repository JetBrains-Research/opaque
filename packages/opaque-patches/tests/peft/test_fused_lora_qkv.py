...
class TestFusedLoRAQKV:
    ...
    def test_fused_lora_qkv_forward(self, device):
        ...
        Q, K, V = Opaque_LoRA_QKV.apply(x, Wq, Aq, Bq, scaling, Wk, Ak, Bk, scaling, Wv, Av, Bv, scaling)
        ref_q = F.linear(x, Wq) + x @ Aq @ Bq * scaling
        ref_k = F.linear(x, Wk) + x @ Ak @ Bk * scaling
        ref_v = F.linear(x, Wv) + x @ Av @ Bv * scaling
        assert torch.allclose(Q, ref_q, rtol=0.001, atol=0.001), f'Fused LoRA QKV Q mismatch: max diff {(Q - ref_q).abs().max():.2e}'
        assert torch.allclose(K, ref_k, rtol=0.001, atol=0.001), f'Fused LoRA QKV K mismatch: max diff {(K - ref_k).abs().max():.2e}'
        assert torch.allclose(V, ref_v, rtol=0.001, atol=0.001), f'Fused LoRA QKV V mismatch: max diff {(V - ref_v).abs().max():.2e}'
    ...
    def test_auto_fuse_qkv_on_get_peft_model(self, device):
        ...
        lora_config = LoraConfig(r=8, lora_alpha=16, lora_dropout=0.0, target_modules=['q_proj', 'k_proj', 'v_proj'])
        apply_model_patches(model, performance=False, compat=True)
        model = get_peft_model(model, lora_config).to(device)
        layers = model.model.model.layers
        for layer in layers:
            attn = layer.self_attn
            assert hasattr(attn, '_opaque_fused_qkv'), 'Attention should have _opaque_fused_qkv after auto-fuse'
    ...
    def test_qwen2_skips_qkv_fusion(self, device):
        ...
        for layer in layers:
            attn = layer.self_attn
            assert not hasattr(attn, '_opaque_fused_qkv'), 'Qwen2 attention should NOT have fused QKV (bias=True)'
    ...
    def test_qwen3_skips_qkv_fusion(self, device):
        ...
        for layer in layers:
            attn = layer.self_attn
            assert not hasattr(attn, '_opaque_fused_qkv'), 'Qwen3 attention should NOT have fused QKV (q_norm/k_norm)'
    ...
    def test_patch_lora_model_manual_qkv(self, device):
        ...
        layers = model.model.model.layers
        assert not hasattr(layers[0].self_attn, '_opaque_fused_qkv'), 'QKV should not be fused before patch_lora_model()'
        patch_lora_model(model)
        assert hasattr(layers[0].self_attn, '_opaque_fused_qkv'), 'QKV should be fused after patch_lora_model()'
    ...
    def test_fused_qkv_clipped_grad(self, device):
        ...
        def per_example_loss(trainable_params, frozen_params, ids, mask, lbls):
            ...
            return outputs.loss
        grad_fn, clip_state = clipped_grad(per_example_loss, argnums=0, batch_argnums=(2, 3, 4), clipping_norm=1.0)
        grads, state = grad_fn(trainable, frozen, input_ids, attention_mask, labels, state=clip_state)
