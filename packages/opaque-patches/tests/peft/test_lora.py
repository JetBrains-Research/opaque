...
class TestLoRAPatches:
    ...
    def test_lora_forward_matches_peft(self, device):
        ...
        lora_layer = PeftLoRALinear(base_linear, 'default', r=rank, lora_alpha=16, lora_dropout=0.0).to(device)
        ...
        assert torch.allclose(out, ref, rtol=RTOL, atol=ATOL), f'LoRA forward mismatch: max diff {(out - ref).abs().max():.2e}'
    ...
    def test_lora_forward_with_bias(self, device):
        ...
        lora_delta = F.linear(F.linear(x, A_weight), B_weight) * scaling
        ref = base_out + lora_delta
        assert torch.allclose(out, ref, rtol=RTOL, atol=ATOL), f'LoRA with bias mismatch: max diff {(out - ref).abs().max():.2e}'
