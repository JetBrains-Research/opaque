...
class TestCrossEntropyPatches:
    ...
    def test_causal_lm_loss_matches_pytorch(self, device):
        ...
        assert torch.allclose(loss, ref, rtol=0.001, atol=0.001), f'Cross-entropy loss mismatch: got {loss.item():.6f}, expected {ref.item():.6f}'
    ...
    def test_causal_lm_loss_with_num_items_in_batch(self, device):
        ...
        loss = _opaque_causal_lm_loss(logits, labels, vocab_size, num_items_in_batch=num_items)
        labels_ref = nn.functional.pad(labels, (0, 1), value=-100)
        shift_labels = labels_ref[..., 1:].contiguous()
        logits_flat = logits.float().view(-1, vocab_size)
        shift_labels_flat = shift_labels.view(-1)
        ref = F.cross_entropy(logits_flat, shift_labels_flat, ignore_index=-100, reduction='sum') / num_items
        assert torch.allclose(loss, ref, rtol=0.001, atol=0.001), f'Sum-reduced loss mismatch: got {loss.item():.6f}, expected {ref.item():.6f}'
