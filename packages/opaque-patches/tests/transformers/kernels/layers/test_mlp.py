...
class TestMLPPatches:
    ...
    def test_swiglu_mlp_matches(self, device):
        ...
        assert torch.allclose(out, ref, rtol=RTOL, atol=ATOL), f'LlamaMLP output mismatch: max diff {(out - ref).abs().max():.2e}'
    ...
    def test_geglu_exact_mlp_matches(self, device):
        ...
        assert torch.allclose(out, ref, rtol=RTOL, atol=ATOL), f'GemmaMLP output mismatch: max diff {(out - ref).abs().max():.2e}'
    ...
    def test_geglu_approx_mlp_matches(self, device):
        ...
        assert torch.allclose(out, ref, rtol=RTOL, atol=ATOL), f'Gemma2MLP output mismatch: max diff {(out - ref).abs().max():.2e}'
