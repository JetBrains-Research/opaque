...
def _assert_per_group_stddev_matches_expected(
    backend_case, grad_template, *, key_seed: int
) -> None:
    ...
    grads = clipped(
        {
            "w": backend_case.array(
                np.random.RandomState(42).randn(4, 3).astype(np.float32)
            ),
            "b": backend_case.array(
                np.random.RandomState(42).randn(4).astype(np.float32)
            ),
        },
        max_norm=pg,
    )
...
class TestMfNoisePerGroupSingleStream:
    ...
    def grad_template(self, backend_case):
        return {
            "w": backend_case.array(np.zeros((4, 3), dtype=np.float32)),
            "b": backend_case.array(np.zeros(4, dtype=np.float32)),
        }

    def test_returns_per_group_noise_stddev(self, backend_case, grad_template):
        _assert_per_group_stddev_matches_expected(
            backend_case, grad_template, key_seed=7
        )

    def test_single_stream_per_group_returns_per_group_stddev(
        self, backend_case, grad_template
    ):
        """Plan checklist name — same assertions as ``test_returns_per_group_noise_stddev``."""
        _assert_per_group_stddev_matches_expected(
            backend_case, grad_template, key_seed=701
        )

    def test_mlp_group_has_larger_noise_stddev_than_attn(
        self, backend_case, grad_template
    ):
        strategy = identity_strategy()
        noise_fn, state = mf_gaussian_noise(
    ...
    def test_constant_max_norm_latch_pergroup_mismatch(
        self, backend_case, grad_template
    ):
    ...
    def test_constant_max_norm_latch_kind_mismatch(self, backend_case, grad_template):
        strategy = band_mf_strategy(bands=3, momentum=0.9)
        noise_fn, state = mf_gaussian_noise(
            grad_template,
            strategy,
    ...
    def test_per_group_non_dict_grads_raises(self, backend_case, grad_template):
        strategy = identity_strategy()
        noise_fn, state = mf_gaussian_noise(
            grad_template,
            strategy,
        ...
        bad = clipped(
            (
                backend_case.array(np.zeros(3, dtype=np.float32)),
                backend_case.array(np.zeros(3, dtype=np.float32)),
            ),
            max_norm=pg,
        )
    ...
    def test_nested_per_group_noise(self, backend_case):
        ...
        assert not np.allclose(
            backend_case.to_host(out.pytree["layer"]["w"]),
            backend_case.to_host(grads["layer"]["w"]),
        )
    ...
    def test_per_group_runs_all_strategies(
        self, backend_case, grad_template, make_strategy
    ):
        ...
        assert isinstance(out.noise_stddev, PerGroup)
        assert not np.allclose(
            backend_case.to_host(out.pytree["w"]),
            backend_case.to_host(grads.pytree["w"]),
        )

    def test_constant_max_norm_latch_pergroup(self, backend_case, grad_template):
    ...
    def test_per_group_matches_isotropic_when_uniform(self, backend_case):
    ...
    def test_per_group_unequal_bounds_strict_utility_win(
        self, backend_case, grad_template
    ):
        ...
        v_opt = sig.for_path("w") ** 2 * backend_case.to_host(grad_template["w"]).size
        v_opt += sig.for_path("b") ** 2 * backend_case.to_host(grad_template["b"]).size
        sigma_iso = nm * pg.effective
        n_w = backend_case.to_host(grad_template["w"]).size
        n_b = backend_case.to_host(grad_template["b"]).size
...
class TestMfNoisePerGroupPairedStream:
    ...
    def test_paired_returns_per_group_stddevs(self, backend_case, grad_template):
        n_steps = 20
        ...
        sq = {
            k: v * v
            for k, v in {
                "w": backend_case.array(np.ones((4, 3), dtype=np.float32)),
                "b": backend_case.array(np.ones(4, dtype=np.float32)),
            }.items()
        }
        sq_pg = pg * pg
...
class TestMfNoisePerGroupMahalanobisSingleStream:
    ...
    def grad_template(self, backend_case):
        return {
            "w": backend_case.array(np.zeros(3, dtype=np.float32)),
            "b": backend_case.array(np.zeros(3, dtype=np.float32)),
        }

    def test_mahalanobis_equals_c1_over_nm_squared(self, backend_case, grad_template):
        n_steps = 12
        strategy = band_mf_strategy(bands=3, momentum=0.9)
        nm = 0.75
        c1 = _max_column_norm(strategy, n_steps=n_steps)
    ...
    def test_pld_match_per_group_single_stream(self, backend_case, grad_template):
        """Plan checklist name — same Mahalanobis check as
        ``test_mahalanobis_equals_c1_over_nm_squared``."""
        self.test_mahalanobis_equals_c1_over_nm_squared(backend_case, grad_template)


class TestPerGroupPairedWithClippedGradAndMf:
    """End-to-end: ``clipped_grad(..., second_moment=True, PerGroup)`` → ``mf_gaussian_noise``."""

    def test_per_group_paired_with_mf(self, backend_case):
        params = {
            "w": backend_case.array(
                np.random.RandomState(42).randn(3).astype(np.float32)
            ),
            "b": backend_case.array(
                np.array(np.random.RandomState(42).randn(), dtype=np.float32)
            ),
        }
        batch_size = 6
        x = backend_case.array(
            np.random.RandomState(42).randn(batch_size, 3).astype(np.float32)
        )
        y = backend_case.array(
            np.random.RandomState(42).randn(batch_size).astype(np.float32)
        )
        ...
        grad_template = {
            "w": backend_case.array(np.zeros(3, dtype=np.float32)),
            "b": backend_case.array(np.zeros((), dtype=np.float32)),
        }
        strategy = band_mf_strategy(bands=4, momentum=0.9)
        second = band_mf_strategy(bands=4, momentum=0.99)
        nm = 1.0
