...
def test_row_l2_lookup_raises_past_horizon():
    # ``row_l2_at`` publishes realized sigma on NoisedPytree.noise_stddev; its
    # own horizon guard is defence-in-depth alongside the engine's and must
    # raise rather than clamp or IndexError.
...
def test_plan_engine_rejects_non_plan_noising_inputs():
    for noising in (torch.eye(3, dtype=torch.float64), object()):
        with pytest.raises(TypeError, match=r"must be MfExecutionPlan"):
            _matrix_factorization_noise({"w": torch.zeros(8)}, noising, key=key(0))
