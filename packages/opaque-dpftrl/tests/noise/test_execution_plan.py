...
def test_plan_vectors_must_be_one_dimensional_and_finite():
    with pytest.raises(ValueError, match="one-dimensional"):
        toeplitz_execution_plan(np.eye(3))
    with pytest.raises(ValueError, match="only finite values"):
        toeplitz_execution_plan([1.0, np.inf, 0.0])
    with pytest.raises(ValueError, match="only finite values"):
        toeplitz_execution_plan([1.0, np.nan, 0.0])


def test_toeplitz_plan_validates_inverse_hint():
    coefficients = [1.0, 0.5, 0.25]
    with pytest.raises(ValueError, match="not the Toeplitz inverse"):
        toeplitz_execution_plan(coefficients, inverse_coefficients=[1.0, 1.0])
    with pytest.raises(ValueError, match="non-empty one-dimensional"):
        toeplitz_execution_plan(coefficients, inverse_coefficients=np.eye(2))
