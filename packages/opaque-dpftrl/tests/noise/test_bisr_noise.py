...
class TestBisrStrategy:
    ...
    def test_plan_row_norms_match_materialized_inverse(self, normalized):
        """Closed-form plan row L2s equal the dense C^{-1} row norms."""
        n_steps = 12
        strategy = bisr_strategy(bandwidth=3, normalized=normalized)
        plan = strategy.execution_plan(n_steps=n_steps)
        dense = strategy.streaming_matrix(n_steps=n_steps).materialize(n_steps)
        np.testing.assert_allclose(
            plan.row_l2, np.linalg.norm(dense, axis=1), atol=1e-10, rtol=1e-10
        )

    def test_plan_supports_horizon_shorter_than_bandwidth(self):
        """A bandwidth-4 recipe still plans at n_steps=2: the banded inverse
        is validated (and truncated) against the plan's own horizon, where
        the n x n Toeplitz truncation is exact."""
        n_steps = 2
        strategy = bisr_strategy(bandwidth=4, normalized=False)
        plan = strategy.execution_plan(n_steps=n_steps)
        dense = strategy.streaming_matrix(n_steps=n_steps).materialize(n_steps)
        np.testing.assert_allclose(
            plan.row_l2, np.linalg.norm(dense, axis=1), atol=1e-10, rtol=1e-10
    ...
    def test_plan_inverse_is_exactly_banded(self):
        """The BISR plan keeps C^{-1} exactly banded past the bandwidth."""
        strategy = bisr_strategy(bandwidth=3)
        plan = strategy.execution_plan(n_steps=16)
        assert plan.inverse_bands <= 3
        assert all(value == 0.0 for value in plan.inverse_coefficients[3:])
