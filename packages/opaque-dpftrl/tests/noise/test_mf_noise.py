...
class TestPldEquivalence:
    ...
    def test_band_mf_pld(self):
        s = band_mf_strategy(n_steps=100, bands=10, momentum=0.95)
        eps_new = acc.band_mf(1.0, sensitivity=s.sensitivity).epsilon_at(self.delta)
    ...
    def test_blt_pld(self):
        s = blt_strategy(n_steps=100, min_sep=25, max_participations=4, momentum=0.95)
        eps_new = acc.blt(1.0, sensitivity=s.sensitivity).epsilon_at(self.delta)
        assert eps_new > 0

    def test_lambda_cgd_pld(self):
        s = lambda_cgd_strategy(0.9, n_steps=100, min_sep=25, max_participations=4)
        eps_new = acc.lambda_cgd(1.0, sensitivity=s.sensitivity).epsilon_at(self.delta)
        assert eps_new > 0

    def test_bisr_pld(self):
        s = bisr_strategy(bandwidth=4, n_steps=100, min_sep=25, max_participations=4)
        eps_new = acc.bisr(1.0, sensitivity=s.sensitivity).epsilon_at(self.delta)
        assert eps_new > 0
...
class TestMfNoise:
    ...
    def _bounded_grad(self):
        return bounded(self.grad, bound=1.0)
    ...
    def test_band_mf_noise(self):
        ...
        noisy, ns2 = nf(self._bounded_grad(), ns)
        assert ns2._step_counter == 1
        assert isinstance(noisy, NoisyPytree)
        assert noisy.bound == pytest.approx(1.0)
        assert noisy.noise_stddev == pytest.approx(1.0)
        assert "w" in noisy.pytree
        assert noisy.pytree["w"].shape == self.grad["w"].shape
    ...
    def test_noise_is_nonzero(self):
        ...
        s = band_mf_strategy(n_steps=50, bands=10, momentum=0.95)
        nf, ns = mf_noise(self.grad, s, noise_multiplier=1.0, key=key(42))
        noisy, _ = nf(self._bounded_grad(), ns)
        diff = torch.norm(noisy.pytree["w"] - self.grad["w"])
        assert diff > 0.1

    def test_raw_grads_are_rejected(self):
        s = band_mf_strategy(n_steps=50, bands=10, momentum=0.95)
        nf, ns = mf_noise(self.grad, s, noise_multiplier=1.0, key=key(42))
        with pytest.raises(TypeError, match="BoundedPytree"):
            nf(self.grad, ns)
