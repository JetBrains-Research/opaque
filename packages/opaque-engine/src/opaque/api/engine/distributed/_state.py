...
def _ensure_backend_if_any(value: Any) -> None:
    """Activate a backend for ``value`` when one is identifiable.

    Distributed helpers are documented as safe outside a process group;
    payloads with no backend-bearing arrays (or calls before any provider
    activation) fall through to the single-process early-outs instead of
    raising.
    """
    try:
...
def assert_scalar_equal(
    ...
    *,
    ...
    Values are gathered as Python objects rather than reduced through a
    fixed-width tensor dtype, so integers compare exactly across the full
    unsigned 64-bit range (RNG-key seeds routinely exceed ``2**63 - 1``)
    and floats keep Python float64 precision.
    """
    if not is_distributed():
        return

    values = runtime.distributed_all_gather_object(value)
    ...
    *,
    ...
    def _accumulate(leaf: Any) -> Any:
        ...
        if ops.is_array(leaf):
            # Accumulate in the provider's widest accumulation dtype so
            # cross-rank divergence below the leaf dtype's resolution is
            # still fingerprinted (float64 on torch CPU/CUDA).
            widened = ops.astype(ops.detach(leaf), ops.accumulator_dtype(leaf))
            total += float(ops.scalar_item(ops.sum(widened)))
        return leaf
...
def reduce_scalar(
    ...
    Safe to call outside a process group (and before any backend is
...
def gather_pytree(pytree: Any, dim: int = 0) -> Any:
    ...
    if len(present_schemas) != len(schemas):
        # Object collectives pickle payloads verbatim, and device arrays
        # unpickle onto the *sender's* device. Stage leaves on the host
        # before the gather and re-home the merged result locally.
        staged = pytree
        if pytree is not None:
            staged = tree_map(
                lambda leaf: (
                    ops.transfer(ops.detach(leaf), "cpu")
                    if ops.is_array(leaf)
                    else leaf
                ),
                pytree,
            )
        gathered = runtime.distributed_all_gather_object(staged)
        merged = _merge_gathered_pytrees(gathered, dim=dim)
        if merged is None:
            return None
        reference = next((leaf for leaf in local_leaves if ops.is_array(leaf)), None)
        device = getattr(reference, "device", None)
        if device is None:
            return merged
        return tree_map(
            lambda leaf: ops.transfer(leaf, device) if ops.is_array(leaf) else leaf,
            merged,
        )
...
def _validate_field_op(field_name: str, field_op: Any) -> None:
    ...
    Callables are checked against the one supported signature, ``fn(value)``.
    Introspection deliberately does not follow
    ...
    try:
        ...
        raise ValueError(
            f"field_ops[{field_name!r}] must be callable as fn(value), "
...
def sync_object(
    state: Any,
    field_ops: Mapping[str, str | Callable[[Any], float | int | None]],
) -> Any:
    ...
    **Callable semantics:** a callable must accept exactly ``fn(value)``
