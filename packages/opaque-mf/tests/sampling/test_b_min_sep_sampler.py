...
def test_different_keys():
    ds = TensorDataset(torch.randn(200, 3))
    a = list(BMinSepSampler(ds, bands=4, sampling_prob=0.1, iterations=30, key=key(1)))
    b = list(BMinSepSampler(ds, bands=4, sampling_prob=0.1, iterations=30, key=key(2)))
    assert a != b
...
def test_same_index_respects_min_separation():
    ...
    batches = list(
        BMinSepSampler(
            ds, bands=bands, sampling_prob=0.35, iterations=120, key=key(101)
        )
    )
...
def test_fold_in_changes_stream():
    ...
    a = list(BMinSepSampler(ds, bands=2, sampling_prob=0.2, iterations=15, key=key(0)))
    b = list(
        BMinSepSampler(
            ds, bands=2, sampling_prob=0.2, iterations=15, key=fold_in(key(0), 1)
        )
    )
    assert a != b
...
def test_invalid_sampling_prob():
    ...
    with pytest.raises(ValueError, match="sampling_prob"):
        BMinSepSampler(ds, bands=2, sampling_prob=0.0, iterations=1, key=key(0))
