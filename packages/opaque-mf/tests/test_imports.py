...
``opaque_accounting`` native module (it is lazy-loaded inside calibration
...
def test_mf_import_does_not_load_native_accounting():
    ...
    for m in [m for m in sys.modules if m.startswith("opaque.mf") or m == "opaque_accounting"]:
        del sys.modules[m]
    import opaque.mf  # noqa: F401

    assert "opaque_accounting" not in sys.modules, (
        "opaque.mf top-level import must lazy-load opaque_accounting"
    )
