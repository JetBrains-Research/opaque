...
def get_eval_params(state: ScheduleFreeState) -> Any:
    """Return the published / evaluation weights ``x`` from the state."""
    return state.x
    ...
    *,
    ...
    Args:
        base: Any ``GradientTransformation`` (e.g.
            :func:`opaque.optimizers.adamw`, :func:`opaque.optimizers.lion`,
    ...
    Returns:
        ...
        :class:`ScheduleFreeState`.  Use
        :func:`get_eval_params` on the state to retrieve the published
        weights for saving / evaluation.
    """
    if not 0.0 <= beta <= 1.0:
...
__all__ = ["schedule_free", "ScheduleFreeState", "get_eval_params"]
