...
from opaque.api.engine.optimizers._radam import _rectification, _rho_t
...
class TestValidation:
    def test_eps_positive(self, params):
        with pytest.raises(ValueError, match="invalid RAdam|eps"):
            radam(params, eps=0.0)

    def test_betas_two_values(self, params):
        with pytest.raises(ValueError, match="invalid RAdam|betas"):
    ...
    def test_negative_weight_decay(self, params):
        with pytest.raises(ValueError, match="invalid RAdam|weight_decay"):
            radam(params, weight_decay=-1.0)

    def test_update_rms_clip_positive(self, params):
        with pytest.raises(ValueError, match="update_rms_clip|invalid RAdam"):
