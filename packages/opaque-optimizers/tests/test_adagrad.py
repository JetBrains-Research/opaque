...
from opaque.optimizers import adagrad, apply_updates
...
def _run_constant(g: float, steps: int, sigma: float | None, **factory_kwargs):
    """Drive Adagrad for ``steps`` steps on a constant gradient ``g``.
    ...
    step, state = adagrad(params, **factory_kwargs)
    updates = None
...
class TestVanilla:
    ...
    def test_matches_torchopt_adagrad(self, params, kwargs):
        ...
        for _ in range(10):
            ...
            for k in params:
                torch.testing.assert_close(params_opaque[k], params_ref[k])

    def test_v_acc_starts_at_initial_value(self, params):
        _step, state = adagrad(params, lr=1e-2, initial_accumulator_value=0.5)
        st = state
        for k in params:
...
class TestDPCorrection:
    ...
    def test_correction_removes_noise_variance_from_denominator(self, sigma, steps):
        ...
        g = 1.0
        u_on = _run_constant(
            g, steps, sigma, lr=_DP_LR, eps=_DP_EPS, noise_bias_correction=True
        )
        u_off = _run_constant(
            g, steps, sigma, lr=_DP_LR, eps=_DP_EPS, noise_bias_correction=False
        )
    ...
    def test_noise_dominant_regime_falls_back_to_uncorrected_v(self):
        ...
        ``v_acc − Φ_acc ≤ 0`` carries no signal, so the ``where(corrected > 0,
        corrected, v_acc)`` branch returns the uncorrected accumulator — hence
        bit-equality with plain Adagrad.  The trailing magnitude bound
        separates that from a floor clamp, which would divide by ``≈ eps`` and
        inflate the coordinate by ~1e8.
        """
        g, sigma, steps, lr = 0.01, 1e3, 10, 1e-3
        u_bc = _run_constant(
            g, steps, sigma, lr=lr, eps=_DP_EPS, noise_bias_correction=True
        )
        u_plain = _run_constant(
            g, steps, None, lr=lr, eps=_DP_EPS, noise_bias_correction=False
        )
