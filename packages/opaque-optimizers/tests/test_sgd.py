"""Tests for opaque.api.optimizers._sgd.
...
from opaque.api.optimizers import sgd
...
class TestSchedule:
    ...
    def test_warmup_step_zero_lr_yields_zero_update(self, params, grads):
        ...
        # The schedule keeps advancing: the next step applies a real lr.
        updates, state = step(
            {k: v.clone() for k, v in grads.items()}, state, params=params
        )
