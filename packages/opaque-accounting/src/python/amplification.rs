...
#[pyfunction]
#[pyo3(name = "cgf_poisson_gaussian_pld", signature = (noise_multiplier, rate))]
pub fn py_cgf_poisson_gaussian_pld(
    noise_multiplier: f64,
    rate: f64,
) -> PyResult<PyPld> {
    let pld = crate::amplification::cgf_poisson_gaussian_pld(noise_multiplier, rate)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    Ok(PyPld::new(pld))
}
