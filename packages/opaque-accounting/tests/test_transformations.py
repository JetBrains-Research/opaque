...
class TestAdaclipConstructor:
    ...
    def test_returns_adaclip(self):
        result = acc.adaclip(acc.gaussian(0.8), batch_size=1000)
    ...
    def test_rejects_non_gaussian(self):
        with pytest.raises(TypeError, match="Gaussian"):
            acc.adaclip(acc.eps_delta(1.0), batch_size=100)  # type: ignore[arg-type]
...
class TestEffectiveNoiseMultiplier:
    ...
    def test_effective_noise_multiplier_is_less_than_base(self):
        ...
        ac = acc.adaclip(acc.gaussian(1.0), batch_size=100)
    ...
    def test_effective_noise_multiplier_formula(self):
        """Verify z_eff = 1 / sqrt(1/z^2 + 1/(4*sigma_b^2))."""
        z = 1.1
        multiplier = 0.05
        batch_size = 200
    ...
    def test_large_batch_size_negligible_cost(self):
        """With large batch_size, sigma_b is large → z_eff ≈ z."""
        z = 1.0
        ac = acc.adaclip(acc.gaussian(z), batch_size=100_000)
        # sigma_b = 100000 * 0.05 = 5000, 1/(4*5000^2) ≈ 0 → z_eff ≈ z
        assert abs(ac.effective_noise_multiplier - z) < 1e-4

    def test_small_batch_size_significant_cost(self):
        """With small batch_size, sigma_b is small → z_eff << z."""
        z = 1.0
        ac = acc.adaclip(acc.gaussian(z), batch_size=5)
        # sigma_b = 5 * 0.05 = 0.25
    ...
    def test_poisson_uses_effective_noise_multiplier(self):
        ...
        ac = acc.adaclip(base, batch_size=100)
        ...
        eps_ac = step_ac.cgf().epsilon_at(1e-5)
        eps_eff = step_eff.cgf().epsilon_at(1e-5)
