...
class TestCyclicPoissonDataclass:
    ...
    def test_matches_manual_poisson_composition(self):
        ...
        eps_proc = proc.pmf().epsilon_at(1e-5)
    ...
    def test_more_groups_higher_epsilon(self):
        ...
        eps_small = acc.cyclic_poisson(acc.band_mf(1.0, 100, 50), 0.01).pmf().epsilon_at(1e-5)
        # More groups (smaller bands)
        eps_large = acc.cyclic_poisson(acc.band_mf(1.0, 100, 5), 0.01).pmf().epsilon_at(1e-5)
