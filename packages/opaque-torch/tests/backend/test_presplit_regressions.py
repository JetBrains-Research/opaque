...
def test_state_dict_resolves_tensors_before_backend_activation() -> None:
    ...
    Runs in a subprocess that never imports the Torch provider package:
    importing it registers the tensor handlers eagerly, which would
    resolve the leaf through the registry and leave the fallback path —
    the thing under test — unexercised.
    """
    import subprocess
    import sys
    ...
    script = (
        "import sys\n"
        "import torch\n"
        "import opaque.ops  # engine only: registers the fallback resolver\n"
        "from opaque.api.engine.backend import active_backend\n"
        "from opaque.serialization import from_state_dict, state_dict\n"
        "assert active_backend() is None\n"
        "assert 'opaque.api.torch' not in sys.modules, sorted(\n"
        "    m for m in sys.modules if m.startswith('opaque.api.torch')\n"
        ")\n"
        ...
        "assert active_backend() is not None and active_backend().name == 'torch'\n"
        "print('ok')\n"
    )
