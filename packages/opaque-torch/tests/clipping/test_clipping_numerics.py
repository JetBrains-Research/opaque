...
from opaque.api.torch.backend import _core
...
def test_bound_holds_without_a_float64_accumulator(monkeypatch, n):
    ...
    monkeypatch.setattr(_core, "_SQ_NORM_ACCUM_DTYPE", torch.float32)
...
def test_blocked_reduction_traces_in_a_full_graph(n):
    ...
    def reduce(leaf):
        ...
        terms = _core._reduction_terms(leaves)
        total, _ = _core.squared_l2_norms(leaves, None, dtype=torch.float32)
        return total + terms
