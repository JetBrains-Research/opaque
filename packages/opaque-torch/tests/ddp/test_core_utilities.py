...
class TestNonDistributed:
    ...
    def test_reduce_scalar_integer_mean_returns_float_without_distribution(self):
        # The result type must not depend on whether a process group is
        # live: distributed integer means are floats (exact sum / world
        # size), so the single-process path returns a float too.
...
class TestSyncObjectSchema:
    ...
    def test_rejects_callable_with_unsupported_signature(self):
        with pytest.raises(ValueError, match=r"callable as fn\(value\)"):
            sync_object(
                self._BooleanState(enabled=True),
                {"enabled": lambda _value, _extra: 1},
            )
    ...
    def test_callable_signature_is_validated_before_any_field_runs(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match=r"callable as fn\(value\)"):
            sync_object(
                self._State(count=1, label="local"),
                {
                    "count": lambda value: reduced.append(value) or 1,
                    "label": lambda _value, _extra: 1,
                },
            )
...
class TestBoundedGradientAggregation:
    ...
    def test_sum_gradients_inplace_second_moment_clipping_output(self):
        ...
        assert result is None
        assert out.grads.max_norm == 0.5
        assert out.squared_grads.max_norm == 1.0
