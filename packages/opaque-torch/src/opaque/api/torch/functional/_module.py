...
def _adapt_functional_module(
    ...
    def adapted_module(*args: Any, **kwargs: Any) -> Any:
        ...
        if isinstance(output, _MappingOutput):
            # The wrapped values were one level below the top when the
            # batch dimension was squeezed; apply the same top-level
            # squeeze the original Mapping would have received.
            values = {
                key: (
                    torch.squeeze(value, dim=0)
                    if isinstance(value, torch.Tensor) and value.ndim > 0
                    else value
                )
                for key, value in output.values.items()
            }
            return output.output_type(**values)
        return output
