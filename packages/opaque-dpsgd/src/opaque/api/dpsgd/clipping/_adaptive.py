...
def _normal_scalar(*, key: RngKey, like: Any | None = None) -> float:
    """Draw a native scalar and materialize it at the state transition boundary."""
    return float(ops.scalar_item(normal(key, (), like=like)))
...
def _sample_noisy_clipping_rate(
    ...
    *,
    ...
    noise = _normal_scalar(key=fold_in(key, step), like=like) * fraction_noise_std
...
def _adaptive_clipping_norm_update(
    *,
    ...
    if like is None:
        update_factor = exp(exponent)
    else:
        update_factor = float(
            ops.scalar_item(
                ops.exp(ops.scalar(exponent, dtype=ops.real_dtype(like), like=like))
            )
        )
        ...
        if is_per_group and (grad_norms is not None or stats is not None):
            ...
            if (
                ...
                for i, gname in enumerate(sorted(current_pg.values.keys())):
                    ...
                    noise = (
                        _normal_scalar(key=group_key, like=reference)
