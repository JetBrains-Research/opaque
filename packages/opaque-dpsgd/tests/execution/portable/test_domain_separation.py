...
from opaque.random import fold_in, key, normal, split
from opaque.types import clipped


def _mechanism_draw(base, *, root: str, step: int, size: int = 6):
    """A mechanism written the way the docs say to write one."""
    return normal(fold_in(base, root, step), (size,))


class TestMechanismRoots:
    def test_distinct_roots_give_independent_streams(self, backend_case):
        base = key(1234)
        ...
        assert not np.array_equal(
            backend_case.to_host(first), backend_case.to_host(second)
        )
        assert not np.array_equal(
            backend_case.to_host(first[:1]), backend_case.to_host(second[:1])
        )

    def test_unrooted_draws_collide(self, backend_case):
        ...
        """
        base = key(1234)
        mine = normal(fold_in(base, 3), (6,))
        theirs = normal(fold_in(base, 3), (6,))
        assert np.array_equal(backend_case.to_host(mine), backend_case.to_host(theirs))

    def test_researcher_root_misses_every_shipped_mechanism(self, backend_case):
        ...
        for shipped in (GAUSSIAN_STREAM_FOLD, ADAPTIVE_CLIPPING_STREAM_FOLD):
            assert not np.array_equal(
                backend_case.to_host(mine),
                backend_case.to_host(_mechanism_draw(base, root=shipped, step=0)),
            )

    def test_caller_integer_derivations_miss_the_gaussian_stream(self, backend_case):
...
class TestSharedKeyAcrossShippedMechanisms:
    def test_gaussian_noise_does_not_collide_with_a_researcher_mechanism(
        self, backend_case
    ):
            ...
            clipped(
                {
                    "a": backend_case.array(
                        np.zeros(6), dtype=backend_case.dtype("float32")
                    )
                },
                max_norm=1.0,
            ),
        ...
        mine = _mechanism_draw(base, root="mylab.rare_events", step=0)
        assert not np.array_equal(
            backend_case.to_host(noised.pytree["a"]), backend_case.to_host(mine)
        )

        # ...and specifically not the unrooted derivation either, which is what
        # gaussian_noise itself used to draw from.
        assert not np.array_equal(
            backend_case.to_host(noised.pytree["a"]),
            backend_case.to_host(normal(fold_in(base, 0), (6,))),
        )
