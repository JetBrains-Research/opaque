...
- A couple of ``loss_type`` values (``"sigmoid"``, ``"hinge"``).
...
from opaque.api.alignment.loss.dpo.types import DPO_LOSSES
...
def _eager_reference(
    ...
    *,
    ...
    loss_fn = DPO_LOSSES[loss_type]
    return loss_fn(chosen_logratio, rejected_logratio, beta=beta)
...
def test_eager_vs_fused_parity(loss_type: str, chunk_size: int) -> None:
    """Fused (chunked) loss matches the all-at-once eager reference."""
    beta = 0.1
    inputs = _make_inputs(seed=1)

    expected = _eager_reference(**inputs, beta=beta, loss_type=loss_type)
    actual = opaque_fused_linear_dpo_loss(
        **inputs, beta=beta, loss_type=loss_type, chunk_size=chunk_size
    )
...
def test_chunk_size_covers_uneven_partition() -> None:
    ...
    base = opaque_fused_linear_dpo_loss(
        **inputs, beta=beta, loss_type="sigmoid", chunk_size=1
    )
    uneven = opaque_fused_linear_dpo_loss(
        **inputs, beta=beta, loss_type="sigmoid", chunk_size=3
    )
...
@pytest.mark.parametrize("loss_type", ["sigmoid", "hinge"])
@pytest.mark.parametrize("chunk_size", [1, 2])
def test_grad_composability(loss_type: str, chunk_size: int) -> None:
    ...
    def fused_sum(h: torch.Tensor) -> torch.Tensor:
        return opaque_fused_linear_dpo_loss(
            h, **static, beta=beta, loss_type=loss_type, chunk_size=chunk_size
        ).sum()

    def eager_sum(h: torch.Tensor) -> torch.Tensor:
        return _eager_reference(h, **static, beta=beta, loss_type=loss_type).sum()
...
def test_grad_chunk_size_invariance() -> None:
    ...
    def fused_sum(h: torch.Tensor, chunk_size: int) -> torch.Tensor:
        return opaque_fused_linear_dpo_loss(
            h, **static, beta=beta, loss_type="sigmoid", chunk_size=chunk_size
        ).sum()
...
def test_autocast_entry_is_noop_when_inactive_on_cpu() -> None:
    ...
    out = opaque_fused_linear_dpo_loss(**inputs, beta=beta, loss_type="sigmoid")
    ...
    with torch.autocast(device_type="cpu", enabled=False):
        out_disabled = opaque_fused_linear_dpo_loss(
            **inputs, beta=beta, loss_type="sigmoid"
        )
...
def test_autocast_entry_follows_active_cpu_autocast() -> None:
    ...
    beta = 0.1
    inputs = _make_inputs(seed=5, dtype=torch.float32)
    plain = opaque_fused_linear_dpo_loss(**inputs, beta=beta, loss_type="sigmoid")
    with torch.autocast(device_type="cpu", dtype=torch.bfloat16):
        wrapped = opaque_fused_linear_dpo_loss(**inputs, beta=beta, loss_type="sigmoid")
...
def test_odd_batch_dim_raises() -> None:
    ...
    with pytest.raises(ValueError, match="2B"):
        opaque_fused_linear_dpo_loss(**inputs, beta=0.1, loss_type="sigmoid")


def test_rejected_loss_type_raises() -> None:
    """A Tier-3 (rejected) loss_type propagates NotImplementedError."""
    inputs = _make_inputs(seed=8)
    with pytest.raises(NotImplementedError):
        opaque_fused_linear_dpo_loss(**inputs, beta=0.1, loss_type="aot_pair")
