...
class TestDpoSft:
    ...
    def test_sft_ignores_extra_positional_args(self) -> None:
        ...
        out = sft_loss(chosen_logp, rejected_logratio)
    ...
    def test_sft_ignores_extra_keyword_args(self) -> None:
        ...
        out = sft_loss(chosen_logp, beta=0.1, label_smoothing=0.05, discopop_tau=0.05)
    ...
    def test_sft_ignores_both_extra_args(self) -> None:
        ...
        rejected = torch.tensor(0.5)
        out = sft_loss(chosen_logp, rejected, beta=0.2, label_smoothing=0.0)
...
# ---------------------------------------------------------------------------
# Cross-variant: vmap(grad) composed over ipo_loss + squarechipo_loss
# ---------------------------------------------------------------------------
