...
- :mod:`opaque.alignment.dpo` — DPO loss family (per-sequence logp, per-pair
...
- :mod:`opaque.alignment.metric` — shared, detached token-level metrics
...
``opaque.api.alignment.*`` and are surfaced through the method that consumes
them (e.g. ``sequence_logp`` via :mod:`opaque.alignment.dpo`), following the
shared-impl re-import pattern of ``opaque.dpsgd.clipping``.
"""

from importlib.metadata import PackageNotFoundError, version as _pkg_version

from opaque.alignment import data, dpo, metric, sft
