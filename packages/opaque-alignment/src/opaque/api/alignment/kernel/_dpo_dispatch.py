...
:func:`opaque_fused_linear_dpo_loss` is the user-facing entry point. It selects
a DPO loss variant from :data:`opaque.api.alignment.loss.dpo.DPO_LOSSES`, binds
its keyword arguments (``beta`` and, where applicable, ``label_smoothing``),
and delegates the actual work to the reusable chunked core
:func:`opaque.api.alignment.kernel._fused_linear_preference.fused_linear_preference`.
...
from opaque.api.alignment.kernel._utils import follow_autocast
from opaque.api.alignment.loss.dpo.types import resolve_dpo_loss
...
def _bind_per_pair_loss_fn(
    ...
    *,
    ...
    """Resolve a DPO variant and bind its scalar keyword arguments.

    Returns a callable ``(chosen_logratio, rejected_logratio) -> Tensor`` (the
    :class:`~opaque.api.alignment.kernel._fused_linear_preference.PerPairLossFn`
    contract). ``beta`` is bound for every variant; ``label_smoothing`` is bound
    only for variants that accept it (e.g. ``sigmoid``), so variants without it
    are not passed an unexpected keyword.
    """
    loss_fn = resolve_dpo_loss(loss_type)
    ...
    *,
    ...
    Args:
        ...
        beta: DPO temperature (reference-deviation strength). Defaults to
            ``0.1``.
        loss_type: DPO variant name (key of
            :data:`~opaque.api.alignment.loss.dpo.DPO_LOSSES`, e.g. ``"sigmoid"``
            or ``"hinge"``). Tier-3 (rejected) names raise
            :class:`NotImplementedError`; unknown names raise :class:`KeyError`.
            Defaults to ``"sigmoid"``.
    ...
    per_pair_loss_fn = _bind_per_pair_loss_fn(
        loss_type,
        beta=beta,
