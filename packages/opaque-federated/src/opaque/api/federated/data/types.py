...
class Population:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.name, str) or not self.name.startswith("/"):
            raise ValueError(
                f"population name must be a string starting with '/', got {self.name!r}"
            )
        if not isinstance(self.version, str):
            raise TypeError(
                f"population version must be a string, got {type(self.version).__name__}"
            )
        if not self.version:
            raise ValueError("population version must not be empty")
