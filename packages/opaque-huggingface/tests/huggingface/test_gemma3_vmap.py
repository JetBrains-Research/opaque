...
"""Gemma3 text decoder under per-example gradients (vmap + compat patches).

Mirrors ``test_olmo2_vmap.py``. Uses a 2-layer ``Gemma3TextConfig`` so the
sliding (``position_embeddings_local``) and global (``position_embeddings_global``)
RoPE branches both run.
"""
...
from opaque.huggingface.patches._standard_models import vmap_eager_attention_forward


def _tiny_gemma3_for_causal_lm():
    ...
    cfg = Gemma3TextConfig(
        ...
        # Two layers so both `is_sliding` branches in Gemma3DecoderLayer.forward
        # exercise the patched apply_rotary_pos_emb / RMSNorm path.
        num_hidden_layers=2,
...
def test_gemma3_module_eager_attention_is_patched():
    import transformers.models.gemma3.modeling_gemma3 as mg3

    assert mg3.eager_attention_forward is vmap_eager_attention_forward


def test_gemma3_text_clipped_grad(device):
    model = _tiny_gemma3_for_causal_lm().to(device)
    model.train()
...
def test_gemma3_forward_no_grad(device):
...
def test_gemma3_forward_backward(device):
...
def test_gemma3_vmap_forward(device):
