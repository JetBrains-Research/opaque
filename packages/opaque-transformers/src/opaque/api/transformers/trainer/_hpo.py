...
``trainer.train(trial=config_dict)``.
"""
...
def _run_optuna_search(
    ...
    **kwargs: Any,
    ...
    process_index = int(getattr(trainer.args, "process_index", 0) or 0)
    ...
    if world_size > 1:
        if not (
            ...
            raise RuntimeError(
                "Distributed Optuna HPO requires torch.distributed to be initialized."
            )

        if process_index == 0:

            def distributed_objective(trial: Any) -> float | list[float]:
                trainer.objective = None
                # Materialize params from hp_space (HF parity path).
...
def _run_wandb_search(
    ...
    **kwargs: Any,
    ...
    world_size = int(getattr(trainer.args, "world_size", 1) or 1)
    process_index = int(getattr(trainer.args, "process_index", 0) or 0)
    is_distributed_hpo = world_size > 1
    if is_distributed_hpo and not (
        torch.distributed.is_available() and torch.distributed.is_initialized()
    ):
        raise RuntimeError(
            "Distributed W&B HPO requires torch.distributed to be initialized."
        )
    ...
    def objective() -> Any:
        ...
        if is_distributed_hpo:
            trial_payload = [dict(trial_params)]
            torch.distributed.broadcast_object_list(trial_payload, src=0)
            trial_params = trial_payload[0]
    ...
    try:
        if not is_distributed_hpo or process_index == 0:
            if not sweep_id:
        ...
        if is_distributed_hpo:
            ...
            if process_index == 0:
                ...
                for _ in range(n_trials):
                    ...
                    torch.distributed.broadcast_object_list(trial_payload, src=0)
                    trainer.objective = None
                    trainer.train(resume_from_checkpoint=None, trial=trial_payload[0])
                    if getattr(trainer, "objective", None) is None:
                        metrics = trainer.evaluate()
                        trainer.objective = trainer.compute_objective(metrics)
                    _release_memory(trainer)
...
def _sync_ray_trial_gpu_to_args(trainer: Any, kwargs: dict[str, Any]) -> None:
    ...
    try:
        ...
        gpus = 0.0
    setattr(trainer.args, "_n_gpu", max(0.0, gpus))
