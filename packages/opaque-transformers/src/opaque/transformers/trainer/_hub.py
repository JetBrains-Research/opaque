"""Hub integration for DPTrainer — Phase 8.

Provides the five hub methods that mirror HF ``Trainer``:

- ``init_hf_repo`` — create (or validate) the Hub repo, set ``hub_model_id``
- ``push_to_hub`` — end-of-training (or user-triggered) upload
- ``_push_from_checkpoint`` — per-checkpoint async upload
- ``_finish_current_push`` — block until the last async upload completes
- ``create_model_card`` — write ``README.md`` (HF card + Opaque DP section)

All five are implemented as free functions that accept the trainer as their
first argument so they can be mixed in without subclassing overhead.

**Reuse contract**:

- ``PushInProgress``         — ``transformers.utils.hub.PushInProgress``
...
- ``HubStrategy``            — ``transformers.trainer_utils.HubStrategy``
...
if TYPE_CHECKING:
    from opaque.transformers.trainer import DPTrainer  # pragma: no cover
...
def _push_from_checkpoint(trainer: "DPTrainer", checkpoint_folder: str) -> None:
    ...
    Mirrors ``Trainer._push_from_checkpoint``.  Respects:
    - ``hub_strategy="end"`` → skip (push only at training end)
    - ``hub_always_push=False`` → skip if a push is already in flight
    - ``hub_strategy="checkpoint"`` → also upload as ``last-checkpoint/``
    - ``hub_strategy="all_checkpoints"`` → also upload as ``checkpoint-N/``
    """
    ...
    # upload_folder gets the latest weights without a full re-save.
    _copy_model_files(checkpoint_folder, output_dir)
    if trainer._processing_class is not None:
    ...
    if a.hub_strategy in [HubStrategy.CHECKPOINT, HubStrategy.ALL_CHECKPOINTS]:
        path_in_repo = (
            "last-checkpoint"
            if a.hub_strategy == HubStrategy.CHECKPOINT
            else Path(checkpoint_folder).name
        )
        checkpoint_push = _upload_folder(
            repo_id=trainer.hub_model_id,
    ...
    if trainer.push_in_progress is None or trainer.push_in_progress.is_done():
        trainer.push_in_progress = _PushInProgress(push_jobs)
    else:
        trainer.push_in_progress.jobs.extend(push_jobs)


def _copy_model_files(checkpoint_folder: str, output_dir: str) -> None:
    """Copy model-weight files from a checkpoint folder into output_dir.

    Mirrors HF's list of modeling files: safetensors, pytorch_model, config,
    generation_config, plus sharded-index + shard files, plus PEFT adapters.
    """
    import shutil
    ...
    files_to_copy = list(_MODELING_FILES)
...
def _finish_current_push(trainer: "DPTrainer") -> None:
    ...
    Mirrors ``Trainer._finish_current_push``.  Safe to call when no push is
    in progress.
    """
    if not hasattr(trainer, "push_in_progress"):
        return
    if trainer.push_in_progress is not None and not trainer.push_in_progress.is_done():
        log.info(
            "Waiting for the current checkpoint push to finish, this might take a couple of minutes."
        )
        trainer.push_in_progress.wait_until_done()
...
def push_to_hub(
    ...
    **kwargs: Any,
    ...
    # Wait for any in-flight checkpoint push before the final upload.
...
def _build_privacy_summary(trainer: "DPTrainer") -> dict[str, float | str]:
    ...
    # Fall back to args.
    if noise_multiplier == "unknown" and a.privacy_noise_multiplier is not None:
        noise_multiplier = a.privacy_noise_multiplier
    if delta == "unknown" and a.privacy_target_delta is not None:
        delta = a.privacy_target_delta

    clipping_norm: float | str
    cn = a.dp_clipping_norm
    if cn is None:
