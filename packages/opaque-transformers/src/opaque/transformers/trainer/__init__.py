...
from opaque.transformers.trainer._dataloader import (
    OpaqueEpochPoissonBatchSampler,
    OpaqueEpochTruncatedPoissonBatchSampler,
)
from opaque.transformers.trainer import _eval
from opaque.transformers.trainer._callback import build_callback_handler
from opaque.transformers.trainer._config import DPTrainingArguments
from opaque.transformers.trainer._eval import EvalLoopOutput, EvalPrediction
from opaque.transformers.trainer._precision import eval_dtype
from opaque.transformers.trainer._scheduler import (
...
class DPTrainer:
    ...
    def __init__(
        ...
        # HF parity (``Trainer._inner_training_loop`` line ~1369):
        # apply Liger-style kernel patches once at init time, after the
        # model is on its target device but before functionalization.
        # Opaque maps Liger's per-component config to opaque-patches kwargs.
        if self.args.use_liger_kernel:
            from opaque.transformers.trainer._liger import (
                apply_liger_kernel_via_opaque_patches,
            )

            apply_liger_kernel_via_opaque_patches(
                self._model, self.args.liger_kernel_config
            )
        ...
        # log row averages over the right window.  The tensor stays on device so
        # that when DDP is added, ``_nested_gather(tr_loss)`` will work for
        # multi-rank aggregation without sync stalls.
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        - **Accountant on resume** preserves heterogeneous composition:
          ...
          ``dp_target_epsilon`` against that prefix.  Changing
          ``dp_noise_multiplier`` / ``dp_target_epsilon`` between
    ...
    def _set_resolved_privacy_args(
        ...
        *,
        ...
        self.args.privacy_target_delta = float(target_delta)
        self.args.privacy_noise_multiplier = float(noise_multiplier)
        self.args.privacy_noise_multiplier_source = noise_multiplier_source
        self.args.privacy_sample_rate = float(sample_rate)
        self.args.privacy_expected_batch_size = int(expected_batch_size)
        self.args.privacy_total_steps = int(total_steps)
    ...
    def _setup_training(
        ...
        *,
        ...
        on the left, so the final ε reaches ``dp_target_epsilon`` once the run
        completes.
        """
        a = self.args
        ...
        if a.dp_per_group_clipping is not None:
            ...
            clip_norm = per_group(
                trainable_params,
                fallback=a.dp_clipping_norm,
                **a.dp_per_group_clipping,
            )
        ...
        target_delta = (
            a.dp_target_delta
            if a.dp_target_delta is not None
        ...
        noise_multiplier_source = (
            "fixed" if a.dp_noise_multiplier is not None else "calibrated"
        )
    ...
    def training_step(
        ...
        # fp16 dynamic-loss-scale: detect overflow on the (already-unscaled,
        # already-clipped, already-summed) gradient.  ``pre_clipping_transform``
        # ran per-example before the clip-norm so the unscale is done; an
        # inf/nan here reflects a real forward/backward overflow.  Mirrors
        # ``torch.amp.GradScaler.step``: skip the optimizer update and back
        # off the scale.  We over-account (the accountant already advanced
        # for this step) — conservatively safe for the privacy guarantee.
    ...
    def prediction_step(
        ...
        *not* in ``ignore_keys + ["loss"]``, collapsed to a bare
        ...
        if self._preprocess_logits is not None and logits_tuple:
            logits_for_hook: Tensor | tuple[Tensor, ...]
            logits_for_hook = (
                logits_tuple[0] if len(logits_tuple) == 1 else logits_tuple
            )
            processed = self._preprocess_logits(logits_for_hook, labs)
            logits_tuple = processed if isinstance(processed, tuple) else (processed,)
    ...
    def evaluation_loop(
        ...
        *,
        ...
        :class:`~opaque.transformers.trainer._eval._PredictionAccumulator`.
        ...
        while pending is not None:
            ...
            if batch_eval_metrics:
                ...
                if logits is not None and labels is not None:
                    ep = EvalPrediction(
                        predictions=_eval.nested_numpify(logits),
                        label_ids=_eval.nested_numpify(labels),
                        inputs=(
                            _eval.nested_numpify(batch) if include_inputs else None
                        ),
                        losses=(
                            _eval.nested_numpify(
                                loss.detach().to("cpu").reshape(()).repeat(bs)
                            )
        ...
        # streaming).  ``total_samples`` (observed) is still the right
        # divisor for throughput in ``speed_metrics`` since that's the
        # number we actually saw, regardless of the dataset's claim.
        # Resolved before ``finalize`` so we can pass it as the
        # truncation bound (HF: ``nested_truncate(all_*, num_samples)``
    ...
    def _resolve_collate_fn(self, base_collator: Callable | None = None) -> Callable:
        ...
        Returns a thin wrapper around ``self._data_collator`` (HF
        parity: ``DataCollatorWithPadding`` / ``default_data_collator``
        fallback is installed at construction time).  The collator is
        deliberately CPU-only so DataLoader workers never touch CUDA/MPS;
        tensors move to ``self._device`` in ``_prepare_inputs`` on the
        main process.
        """
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if a.dp_sampler == "truncated_poisson":
            sampler = OpaqueEpochTruncatedPoissonBatchSampler(
                dataset=dataset,
                sample_rate=ctx.sample_rate,
                max_batch_size=a.dp_max_batch_size or a.expected_batch_size,
    ...
    def create_scheduler(self, num_training_steps: int) -> Callable[[int], float]:
        ...
        Dispatches via :func:`opaque.transformers.trainer._scheduler.build_lr_schedule`,
        ...
        if a.dp_clipping_mode == "adaptive":
            if loss_scaler is not None:
                ...
                raise NotImplementedError(
                    "fp16=True with dp_clipping_mode='adaptive' is not yet "
                    ...
                    "needed) or dp_clipping_mode='fixed'/'auto' with fp16."
                )
            return adaptive_clipped_grad(
                ...
                target_quantile=1.0 - a.dp_target_clipping_rate,
                clipping_norm_max=a.dp_clipping_norm_max,
                microbatch_size=microbatch_size,
                return_aux=True,
                key=key(a.seed),
                normalize_by=expected_batch_size,
            )
        elif a.dp_clipping_mode == "auto":
            return auto_clipped_grad(
                loss_fn,
                argnums=0,
                batch_argnums=batch_argnums,
                R=clip_norm,
                gamma=a.dp_auto_clipping_gamma,
                normalize_by=expected_batch_size,
                microbatch_size=microbatch_size,
                return_aux=True,
    ...
    def _calibrate_noise(
        ...
        *,
        ...
        equals ``dp_target_epsilon``.
        """
        if a.dp_noise_multiplier is not None:
            log.info("Using fixed noise multiplier: %.4f", a.dp_noise_multiplier)
            return a.dp_noise_multiplier
        ...
        result = cal.calibrate(
            cal.epsilon_budget(a.dp_target_epsilon, delta=target_delta),
            objective,
            param_min=a.dp_calibration_min,
            param_max=a.dp_calibration_max,
            tolerance=a.dp_calibration_tolerance,
        )
        log.info(
    ...
    def _save_checkpoint(self, ctx: "_TrainingContext", step: int) -> str:
        ...
        # Always: model weights via save_pretrained (needs params restored first).
        self._restore_params(ctx.trainable_params)
        self._save_model_artifacts(ckpt_dir)

        # HF parity: register ``best_model_checkpoint`` by *looking up* the
        # folder named ``checkpoint-{best_global_step}`` rather than only
        # recognising the best step when it coincides with this save's step.
        # Without this, an eval boundary that improves the metric at step
        # 100 followed by a save_strategy="steps" boundary at step 200
        # would never register checkpoint-100 as best — and rotation could
        # delete it because no folder is protected.  Resolve *before*
        # writing ``trainer_state.json`` so the file lands once with the
        # final ``best_model_checkpoint`` already populated.
        if self.state.best_global_step is not None:
            ...
            if os.path.isdir(best_dir):
                ...
                log.debug(
                    "best_global_step=%d but no checkpoint-%d/ folder exists "
                    "(best step fell into a non-saved bucket); leaving "
                    "best_model_checkpoint unset",
                    self.state.best_global_step,
                    self.state.best_global_step,
                )

        # Interpretability artifacts (always written, HF parity for the
        # first three plus accountant.json as the DP-correct extension):
        # - ``trainer_state.json`` carries ``best_metric`` /
        #   ``best_model_checkpoint`` / cadence;
        # - ``training_args.bin`` records the training config (HF parity
        #   filename; payload is ``DPTrainingArguments`` which subclasses
        #   ``TrainingArguments``, so HF tooling can read it);
        # - ``accountant.json`` records the privacy guarantee the saved
        #   model satisfies — a property *of the model*, not training
        #   state.  Without it, resume re-enters ε=∞.
        # Written *after* ``best_model_checkpoint`` resolution so
        # ``trainer_state.json`` is produced exactly once per save.
        self._save_trainer_state(ckpt_dir)
        self._save_training_args(ckpt_dir)
        self._save_accountant(ckpt_dir, ctx)
        # Resumability artifacts (gated by ``save_only_model``):
        # optimizer state, DP runtime state, RNG state — all large and
        # only needed to continue training bit-exactly.
        if not a.save_only_model:
            self._save_optimizer(ckpt_dir, ctx)
            self._save_dp_runtime(ckpt_dir, ctx)
            self._save_rng_state(ckpt_dir)

        # Rotation honors ``save_total_limit`` and protects best when set.
    ...
    def _save_rng_state(self, ckpt_dir: str) -> None:
        ...
        # When DDP lands the call site passes (rank, world_size) and the
        # filename becomes ``rng_state_{rank}.pth`` automatically.
        torch.save(
            ckpt.snapshot_rng_state(),
            ckpt.rng_state_path(ckpt_dir),
        )
    ...
    def _load_rng_state(self, ckpt_dir: str) -> None:
        # Resolve via the world-aware helper so DDP-savings (Phase 9) load
        # from the correct per-rank file without the call site changing.
        path = ckpt.rng_state_path(ckpt_dir)
        if not os.path.exists(path):
            return
    ...
    def _warn_on_arg_drift(self, payload: dict[str, Any]) -> None:
        ...
        for arg_name, current in (
            ...
            (
                "target_delta",
                ctx.target_delta if ctx is not None else a.dp_target_delta,
            ),
            (
                "noise_multiplier",
                ctx.noise_multiplier if ctx is not None else a.dp_noise_multiplier,
            ),
            (
                "total_steps",
