"""Tests for HF ``performance_kernels_config`` translation.

Validates that ``opaque.api.transformers.trainer._performance_kernels.translate_performance_kernels_config``
...
2. Unknown keys (``layer_norm`` and any future Liger key with no opaque
   ...
   kwargs by signature inside ``_apply_performance_kernels_to_instance``.
...
from opaque.api.transformers.trainer._performance_kernels import translate_performance_kernels_config


def test_empty_config_yields_empty_dict():
    assert translate_performance_kernels_config(None) == {}
    assert translate_performance_kernels_config({}) == {}


def test_llama_style_keys_translate():
    """Standard Llama Liger config: rope/rms_norm/swiglu/cross_entropy."""
    out = translate_performance_kernels_config(
        {
            "rope": True,
            "rms_norm": True,
            "swiglu": True,
...
def test_gemma_style_keys_geglu_maps_to_fuse_swiglu():
    ...
    out = translate_performance_kernels_config({"rope": True, "geglu": True, "rms_norm": True})
    assert out == {
        "fuse_rope": True,
        "fuse_swiglu": True,
        "fuse_rms_norm": True,
...
def test_unknown_keys_are_dropped_with_info_log(caplog):
    ...
    signature)."""
    with caplog.at_level(logging.INFO, logger="opaque.api.transformers.trainer._performance_kernels"):
        out = translate_performance_kernels_config(
...
def test_falsy_values_pass_through_as_false():
    ...
    assert translate_performance_kernels_config({"rope": False, "rms_norm": False}) == {
        "fuse_rope": False,
        "fuse_rms_norm": False,
    }
...
def test_truthy_non_bool_values_become_true():
    ...
    assert translate_performance_kernels_config({"rope": 1, "rms_norm": "yes"}) == {
        "fuse_rope": True,
        "fuse_rms_norm": True,
    }
