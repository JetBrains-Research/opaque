...
class TestDpFtrlSamplerDispatch:
    ...
    def test_sampler_dispatched_via_auto(
        ...
        args = _args(
            output_dir=str(tmp_path / mechanism),
            mechanism=mechanism,
            max_steps=16 if sampling_mode == "balls_in_bins" else max_steps,
            sampling_mode=sampling_mode,
        )
        torch.manual_seed(0)
        trainer = DPTrainer(
...
class TestDpTrainerIdentityBallsInBins:
    def test_trains_complete_epochs(self, tmp_path):
        args = _args(
            output_dir=str(tmp_path / "identity-balls-in-bins"),
    ...
    def test_rejects_partial_epoch(self, tmp_path):
        args = _args(
            output_dir=str(tmp_path / "identity-balls-in-bins"),
            mechanism="mf_identity",
            sampling_mode="balls_in_bins",
        ...
        with pytest.raises(ValueError, match="multiple of steps_per_epoch"):
            trainer.train()
