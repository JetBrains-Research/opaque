...
class TestLegacyAliases:
    ...
    def test_no_cuda_no_longer_accepted(self):
        # Phase 10 follow-up: ``DPTrainingArguments`` is now standalone (no
        ...
        with pytest.raises(TypeError, match="no_cuda"):
            DPTrainingArguments(no_cuda=True)
...
class TestUnknownKwargs:
    ...
    def test_ddp_backend_unsupported_value_raises(self, backend):
        with pytest.raises(ValueError, match="ddp_backend"):
            DPTrainingArguments(ddp_backend=backend)
...
class TestMaxGradNorm:
    """``max_grad_norm`` is the DP clip bound: scalar or per-group dict."""

    def test_positive_scalar_accepted(self):
        args = DPTrainingArguments(max_grad_norm=2.5)
        assert args.max_grad_norm == 2.5

    def test_non_positive_scalar_raises(self):
        with pytest.raises(ValueError, match="max_grad_norm"):
            DPTrainingArguments(max_grad_norm=0.0)

    def test_dict_fallback_only_normalizes_to_float(self):
        args = DPTrainingArguments(max_grad_norm={"fallback": 2.0})
        assert args.max_grad_norm == 2.0

    def test_dict_per_group_accepted(self):
        args = DPTrainingArguments(
            max_grad_norm={"fallback": 1.0, "linear": 2.0, "bias": 0.5},
        )
        assert args.max_grad_norm == {
            "fallback": 1.0,
            "linear": 2.0,
            "bias": 0.5,
        }
    ...
    def test_json_string_object_parsed(self):
        args = DPTrainingArguments(
            max_grad_norm='{"fallback": 1.0, "x": 2.0}',
        )
        assert args.max_grad_norm == {"fallback": 1.0, "x": 2.0}

    def test_dict_missing_fallback_raises(self):
        with pytest.raises(ValueError, match="fallback"):
            DPTrainingArguments(max_grad_norm={"linear": 1.0})
