...
import opaque.transformers.trainer._checkpoint as ckpt
import opaque.transformers.trainer._hpo as hpo_module
...
def test_reporting_callbacks_receive_privacy_logs_and_functional_slots(
            ...
            records.append(
                {
                    ...
                    "model": kwargs["model"],
                    "privacy_target_delta": args.privacy_target_delta,
                    "privacy_noise_multiplier": args.privacy_noise_multiplier,
                    "privacy_noise_multiplier_source": args.privacy_noise_multiplier_source,
                    "state_id": id(state),
                }
            )
    ...
    assert train_begin["privacy_target_delta"] is not None
    assert train_begin["privacy_noise_multiplier"] == 1.0
...
def test_reporting_callbacks_receive_raw_per_group_privacy_logs(tmp_path, monkeypatch):
    ...
    trainer = DPTrainer(
        ...
        args=_args(
            ...
            report_to="wandb",
            save_strategy="no",
            dp_per_group_clipping={"linear": 1.0},
        ),
        train_dataset=_dataset(),
    )
