...
Covered:
- Each ``(use_cpu, use_mps_device, no_cuda)`` combination resolves to
  the expected device on a CUDA-available host, an MPS-only host, and
  a CPU-only host.
- ``_n_gpu`` is 0 for CPU/MPS, 1 for CUDA.
- ``no_cuda`` deprecation is forwarded to ``use_cpu`` with FutureWarning.
...
# ----------------------------------------------------------------------------
# Helper: build minimal args (skip output_dir / dp_* required fields by
...
# ----------------------------------------------------------------------------
# Deprecation: no_cuda → use_cpu (with FutureWarning)
# ----------------------------------------------------------------------------


def test_no_cuda_forwards_to_use_cpu_with_warning():
