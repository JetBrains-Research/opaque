...
"""Verify `opaque.transformers` auto-patching respects OPAQUE_SKIP_* env vars.
...
def test_default_import_applies_compat_patches():
    ...
    out = _run(
        "import opaque.transformers\nprint(opaque.transformers.is_patched())",
        env_overrides={},
    )
    assert out == "True"
...
def test_skip_transformers_patches_all_reports_no_patches_landed():
    ...
    out = _run(
        "import opaque.transformers\n"
        "print(opaque.transformers.is_patched(), opaque.transformers.is_vmap_patched())",
...
def test_unknown_skip_token_raises():
    ...
    result = subprocess.run(
        [sys.executable, "-c", "import opaque.transformers"],
        capture_output=True,
        text=True,
        env=env,
    )
