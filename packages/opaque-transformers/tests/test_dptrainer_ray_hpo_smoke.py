...
This module lives **outside** ``tests/huggingface/`` on purpose: with pytest's
default ``--import-mode=importlib``, files under ``tests/huggingface/`` are
imported as top-level ``huggingface.*`` modules.  Cloudpickle then encodes
``model_init`` callables / model classes with ``__module__ == "huggingface…"``,
which collides with the Hugging Face ecosystem namespace and breaks Ray Tune
worker unpickling (``No module named 'huggingface.test_hpo'``).  Keeping the
smoke test here yields a stable module path (``test_dptrainer_ray_hpo_smoke``).
"""
