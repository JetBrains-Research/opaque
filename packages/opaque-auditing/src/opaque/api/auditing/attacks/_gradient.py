...
def gradient_scores(
    ...
    *args: Any,
    ...
    # Scoring wants values, not graphs. ``values_only`` is the portable way to
    # say so: the score is a norm of the gradient, read once and discarded, so
    # a provider need not build a differentiable graph over it. Detaching the
    # inputs additionally keeps the provider from recording gradients outside
    # the functional transform.
    grad_fn = grad_and_value(loss_fn, values_only=True)
