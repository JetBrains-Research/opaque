...
def _validate_batch_argnums(batch_argnums: tuple[int, ...], n_non_batch: int) -> None:
    ...
    if len(set(batch_argnums)) != len(batch_argnums):
        ConfigurationError.raise_(f"batch_argnums must be unique, got {batch_argnums}")
    if tuple(sorted(batch_argnums)) != batch_argnums:
        ConfigurationError.raise_(f"batch_argnums must be sorted, got {batch_argnums}")
...
def _scoring_loader(
    *,
    ...
    if batch_size < 1:
        ConfigurationError.raise_(f"batch_size must be positive, got {batch_size}")
