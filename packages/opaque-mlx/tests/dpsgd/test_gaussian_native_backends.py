"""Native MLX Gaussian noise behavior for DP-SGD."""
...
def test_gaussian_noise_with_native_mlx_arrays() -> None:
    grads = {
        "weight": mx.zeros(128, dtype=mx.float32),
        "bias": mx.zeros(16, dtype=mx.float32),
    }
    ...
    bounded, _ = bounded_fn(
        clipped(mx.zeros(4096, dtype=mx.float32), max_norm=1.0), bounded_state
    )
    ...
    dtype_fn, dtype_state = gaussian_noise(
        noise_multiplier=1.0, compute_dtype=mx.float32, key=key(37)
    )
    half = mx.zeros(128, dtype=mx.float16)
    ...
    per_group, per_group_state = per_group_fn(
        clipped(
            {
                "small": mx.zeros(4096, dtype=mx.float32),
                "large": mx.zeros(4096, dtype=mx.float32),
            },
            max_norm=norms,
        ),
        per_group_state,
    ...
    paired, _ = per_group_fn(
        SecondMomentClippingOutput(
            grads=clipped(mx.zeros(128, dtype=mx.float32), max_norm=1.0),
            squared_grads=clipped(mx.zeros(128, dtype=mx.float32), max_norm=1.0),
        ),
        per_group_state,
    )
