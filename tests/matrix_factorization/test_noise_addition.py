...
class TestDenseMatrixFactorizationNoise:
    def test_identity_matrix(self):
        ...
        noise_fn, state = custom_mf_noise(grad, noising, stddev=1.0, seed=42)
...
class TestStreamingMatrixFactorizationNoise:
    ...
    def test_adds_noise(self):
        ...
        grad = torch.zeros(10, dtype=torch.float32)
        noise_fn, state = custom_mf_noise(grad, identity(), stddev=1.0, seed=42)
    ...
    def test_noise_scale(self):
        ...
        # Large stddev
        noise_fn, state = custom_mf_noise(grad, identity(), stddev=100.0, seed=0)
        noisy, _ = noise_fn(grad, state)
        large_std = noisy.std().item()

        # Small stddev
        noise_fn, state = custom_mf_noise(grad, identity(), stddev=1.0, seed=1)
        noisy, _ = noise_fn(grad, state)
