...
class TestEpsilonTarget:
    ...
    def test_decreasing(self):
        ...
        assert cal.epsilon(3.0, delta=1e-5).decreasing is True
    ...
    def test_rejects_negative_epsilon(self):
        with pytest.raises(ValueError, match="epsilon"):
            cal.epsilon(-1.0, delta=1e-5)

    def test_rejects_delta_out_of_range(self):
        with pytest.raises(ValueError, match="delta"):
            cal.epsilon(3.0, delta=0.0)
        with pytest.raises(ValueError, match="delta"):
            cal.epsilon(3.0, delta=1.0)


class TestDeltaTarget:
    def test_valid(self):
        t = cal.delta(1e-5, epsilon=3.0)
        assert isinstance(t, DeltaTarget)
        assert t.value == pytest.approx(1e-5)

    def test_decreasing(self):
        assert cal.delta(1e-5, epsilon=3.0).decreasing is True

    def test_rejects_invalid(self):
        with pytest.raises(ValueError, match="delta"):
            cal.delta(0.0, epsilon=3.0)
        with pytest.raises(ValueError, match="epsilon"):
            cal.delta(1e-5, epsilon=-1.0)
...
class TestAdvantageTarget:
    ...
    def test_decreasing(self):
        assert cal.advantage(0.1).decreasing is True
...
class TestBetaTarget:
    def test_valid(self):
        t = cal.beta(0.05, alpha=0.01)
        assert isinstance(t, BetaTarget)
        assert t.value == pytest.approx(0.05)

    def test_decreasing(self):
        """Beta increases with noise → decreasing=False."""
        assert cal.beta(0.05, alpha=0.01).decreasing is False

    def test_rejects_invalid(self):
        with pytest.raises(ValueError, match="beta"):
            cal.beta(0.0, alpha=0.01)
        with pytest.raises(ValueError, match="alpha"):
            cal.beta(0.5, alpha=0.0)


class TestRiskTarget:
    def test_valid(self):
        t = cal.risk(0.1, prior=0.5)
        assert isinstance(t, RiskTarget)
        assert t.value == pytest.approx(0.1)

    def test_decreasing(self):
        """Risk increases with noise → decreasing=False."""
        assert cal.risk(0.1, prior=0.5).decreasing is False

    def test_rejects_invalid(self):
        with pytest.raises(ValueError, match="risk"):
            cal.risk(0.0, prior=0.5)
        with pytest.raises(ValueError, match="prior"):
            cal.risk(0.1, prior=0.0)
...
class TestCalibrateErrors:
    ...
    def test_param_min_ge_max(self):
        with pytest.raises(ValueError, match="param_min"):
            cal.calibrate(cal.epsilon(3.0, delta=1e-5), self._build, 0.8, 0.5)

    def test_target_outside_bracket(self):
        """Target not achievable within bounds → ValueError."""
        with pytest.raises(ValueError):
            cal.calibrate(
                cal.epsilon(0.001, delta=1e-5), self._build, 0.5, 0.6
            )
...
class TestCalibrateEpsilon:
    ...
    def test_basic(self):
        result = cal.calibrate(
            cal.epsilon(5.0, delta=1e-5), self._build, 0.3, 1.2
        )
    ...
    def test_strict_epsilon(self):
        result = cal.calibrate(
            cal.epsilon(4.0, delta=1e-5), self._build, 0.3, 1.2
        )
        assert result.converged
    ...
    def test_loose_epsilon(self):
        result = cal.calibrate(
            cal.epsilon(8.0, delta=1e-5), self._build, 0.1, 1.0
        )
        assert result.converged
...
class TestCalibrateDifferentBatchSizes:
    ...
    def test_converges(self, batch_size):
        ...
        def build(nm):
            return acc.poisson(acc.gaussian(nm), q) * 1000

        result = cal.calibrate(
            cal.epsilon(5.0, delta=1e-4), build, 0.1, 1.2
        )
        assert result.converged
...
class TestCalibrateAdvantage:
    ...
    def test_roundtrip(self):
        ...
        result = cal.calibrate(
            cal.advantage(0.1), build, 0.3, 1.2
        )
        assert result.converged
...
class TestCalibrateBeta:
    ...
    def test_roundtrip(self):
        ...
        result = cal.calibrate(
            cal.beta(0.5, alpha=0.1), build, 0.3, 1.2
        )
        assert result.converged
    ...
    def test_monotonicity(self):
        ...
        result_low = cal.calibrate(
            cal.beta(0.3, alpha=0.1), build, 0.3, 1.2
        )
        result_high = cal.calibrate(
            cal.beta(0.7, alpha=0.1), build, 0.3, 1.2
        )
