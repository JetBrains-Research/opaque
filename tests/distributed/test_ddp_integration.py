...
class TestGradientAggregation:
    ...
    def test_all_reduce_gradients_sum(self, device):
        ...
        assert torch.allclose(
            result["weight"],
            torch.ones(5, 3, device=device) * expected_sum
        )
        assert torch.allclose(
            result["bias"],
            torch.ones(3, device=device) * expected_sum
        )
    ...
    def test_average_gradients(self, device):
        ...
        assert torch.allclose(
            result["weight"],
            torch.ones(5, 3, device=device) * expected_avg,
            atol=1e-5
        )
        assert torch.allclose(
            result["bias"],
            torch.ones(3, device=device) * expected_avg,
            atol=1e-5
        )
...
class TestStateSynchronization:
    ...
    def test_sync_scalar(self, device):
        ...
        from opaque.distributed import sync_scalar, get_rank, get_world_size
    ...
    def test_sync_adaptive_clip_state(self, device):
        ...
        from opaque.distributed import sync_state, get_rank, get_world_size
