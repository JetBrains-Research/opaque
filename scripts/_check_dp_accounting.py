...
for sigma in [0.1, 0.25, 0.3, 0.35, 0.5, 0.65, 0.8, 1.2]:
    ...
    print(
        f"Gaussian({sigma:4.2f}) eps@1e-5  ref={eps_ref:.10f}  ours={eps_ours:.10f}  diff={diff:.2e}{flag}"
    )
...
for sigma in [0.5, 0.8, 1.2]:
    for q in [0.001, 0.0005, 0.0001]:
        for steps in [10, 50, 200, 500, 1000]:
            ...
            flag = " <<<" if diff > 1e-6 else ""
            print(
                f"Poisson(G({sigma}),{q})*{steps:4d}  ref={eps_ref:.8f}  ours={eps_ours:.8f}  diff={diff:.2e}{flag}"
            )
...
print(
    f"Roundtrip: epsilon_at(1e-5)={eps:.10f}, delta_at(eps)={delta:.2e} (should be ~1e-5)"
)
