#!/usr/bin/env bash
# BATCH 13 — GLUE LR sweep with clipping made inert. Fixes what the smoke run found.
#
...
MAXCONC="${MAXCONC:-3}"; LOGDIR=campaign_logs/batch13; mkdir -p "$LOGDIR"
BASE="--preset roberta-large-glue --glue-task rte --clipping-mode fixed --clipping-norm 1e6"
ROT="--lora-xse-p-e 0.3125 --lora-xse-rotation-step-interval 1"

QUEUE=(
  "glue-rte-xse-lr1e4-s42|$BASE $ROT --learning-rate 1e-4"
  "glue-rte-xse-lr3e4-s42|$BASE $ROT --learning-rate 3e-4"
  "glue-rte-xse-lr1e3-s42|$BASE $ROT --learning-rate 1e-3"
  "glue-rte-xse-lr3e3-s42|$BASE $ROT --learning-rate 3e-3"
)
